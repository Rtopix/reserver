...
bot.once('spawn', () => console.log('✅ Бот заспавнился'));
...
bot.on('kicked', (r) => { console.log('❌ Кик:', r); process.exit(1); });
bot.on('end', (r) => console.log('🔌 Отключён:', r));
bot.on('error', (e) => console.error('⚠️ Ошибка:', e.message));
...
const server = http.createServer((req, res) => {
  if (req.url === '/health') {
    ...
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.end(`Bot running. Auth: ${isAuthenticated}. Uptime: ${Math.floor((Date.now() - lastAction)/1000)}s`);
  } else {
    res.writeHead(404);
    res.end('Not found');
  }
